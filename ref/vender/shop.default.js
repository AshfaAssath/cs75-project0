var jsshop = new function() {
	...
	this.change_num = function(id , num) {
		...
		if(obj_cart_num) {
			...
			if(obj_cart_price) obj_cart_price.innerHTML = "￥"+ kj.toint(val * kj.toint(obj_price.value));
			this.refresh_price();
		}
	}
	//刷新价格
	...
	this.mouseover = function(id) {
		...
		kj.set("#id_nosel_"+id,"className","x_sel1");
		this.orderid = id;
		kj.addClassName(".Tmenu"+id , "x_sel2");
	}
	this.mouseout = function(id) {
		var obj = kj.obj("#id_nosel_"+this.orderid);
		if(obj) obj.className = 'x_nosel';
		kj.delClassName(".Tmenu"+id , "x_sel2");
	}
	...
	this.resize = function(){
		...
		var offset = kj.offset("#id_mainbg");
	...
	this.hash = function (name) {
		...
		if(obj.length<1) {
			...
			this.hashname = name;
			var id = this.shopid;
			kj.ajax.get(kj.cfg('baseurl') + "/index.php?app_act=grouplist&id="+id+"&index_group="+arr[1],function(data){
	...
	this.sort = function (name , val) {
		...
		kj.ajax.get(kj.cfg('baseurl') + "/index.php?app_act=sortlist&id="+this.shopid+"&sort="+name+val,function(data){
			kj.obj("#id_grouplist").innerHTML = data;
			window.location.hash=jsshop.hashname;
			if(jsfooter) jsfooter.align_height();
		});
	...
	this.comment = function(menu_id) {
		kj.dialog({id:'comment' + menu_id,title:'查看评论',url:kj.cfg('baseurl') + '/index.php?app_act=comment&menu_id='+menu_id,w:500,'max_h':650,showbtnhide:false,showbtnmax:false,type:'iframe'});
	}
}
